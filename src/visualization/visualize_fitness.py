...
class Injection(object):
    ...
    def __init__(self, sigma=10.0, num_odes=8, edge=2.5):
        self.num_odes = num_odes

        self.x_array = self.compute_x_array(edge=edge)
        self.sigma = sigma
...
class ExtendedBreadthProcessProtocolData(ProcessProtocolData):
    ...
    def plot(self, i, linestyle='-'):
        plt.plot(self.kl2_array, np.array(self.b_cells[i]) / self.num_gcs, linestyle=linestyle,
    ...
    def plot_protocols(self, set_sns=False):
        ...
        for i in range(len(self.kl1_array)):
            plt.errorbar(self.kl2_matrix[i], self.mean_bnab_array[i] / self.num_gcs,
                         yerr=self.error_bnab_array[i] / self.num_gcs, markerfacecolor='none', marker=marker[i],
